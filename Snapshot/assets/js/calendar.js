	...
	var imgDir = "../assets/img/calendar/";			// directory for images ... e.g. var imgDir="/img/"
    ...
    */
	...
	if (dom){
		...
		document.write ("<div onclick='bShow=true' id='calendar' style='z-index:+999;position:absolute;visibility:hidden;'><table cellspacing='0' width="+((showWeekNumber==1)?250:150)+" style='font-size:13px;border:solid 1px #ccc;' bgcolor='#f9f9f9'><tr style='border-width:1;border-style:solid;border-color:#395d9c;' bgcolor='#F3EFEC'><td><table width='"+((showWeekNumber==1)?248:240)+"'><tr><td style='padding:10px 5px; background:#f9f9f9; font-size:13px;'><font color='#000000'><B><span id='caption'></span></B></font></td><td style='text-align:left; background:#f9f9f9; width:30px;'><a href='javascript:hideCalendar()'><IMG SRC='"+imgDir+"close.png' width='15' height='13' BORDER='0' ALT='Close the Calendar'></a></td></tr></table></td></tr><tr><td style='padding:15px 0 5px 0;' bgcolor=#ffffff><span id='content'></span></td></tr>");
			
		if (showToday==1){
			document.write ("<tr><td style='padding:7px 5px; text-align:center; background:#f9f9f9;'><span id='lblToday'></span></td></tr>");
		}
	
		document.write ("</table></div><div id='selectYear' style='z-index:+999;position:absolute;visibility:hidden;padding-left:0;'></div><div id='selectMonth' style='z-index:+999;position:absolute;visibility:hidden;padding-left:75px;'></div>");
	}
	...
	var	styleLightBorder="border:1px solid #ccc; background-color:#f2f2f2;";
...
//			sHTML1+="<span id='spanLeft'	style='border-width:0;cursor:pointer' onmouseover='swapImage(\"changeLeft\",\"left2.png\");window.status=\""+scrollLeftMessage+"\"' onclick='javascript:decMonth()' onmouseout='clearInterval(intervalID1);swapImage(\"changeLeft\",\"left1.png\");window.status=\"\"' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartDecMonth()\",500)' onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeLeft' SRC='"+imgDir+"left1.png' width=10 height=11 BORDER=0>&nbsp</span>&nbsp;";
			...
			sHTML1+="<span id='spanMonth' style='padding-left:20px;border-width:0;cursor:pointer'	onmouseover='swapImage(\"changeMonth\",\"drop2.png\");window.status=\""+selectMonthMessage+"\"' onmouseout='swapImage(\"changeMonth\",\"drop1.png\");window.status=\"\"' onclick='popUpMonth()'></span>&nbsp;";
			// link to next month 
...
//		var	sTmp;
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			for	(var i=0; i<12;	i++) {
				sName =	monthName[i];
				if (i==monthSelected){
					sName =	"<B style='color:#222;'>" +	sName +	"</B>";
				}
				sHTML += "<tr><td id='m" + i + "' onmouseover='this.style.backgroundColor=\"#f9f9f9\"' onmouseout='this.style.backgroundColor=\"\"' style='cursor:pointer; text-align:center; height:24px; line-height:24px;' onclick='monthConstructed=false;monthSelected=" + i + ";constructCalendar();popDownMonth();event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td></tr>";
			}

			document.getElementById("selectMonth").innerHTML = "<table style='width:44px; border:1px solid #ccc; font-size:11px;' bgcolor='white' cellspacing=0 onmouseover='clearTimeout(timeoutID1)'	onmouseout='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"popDownMonth()\",100);event.cancelBubble=true'>" +	sHTML +	"</table>";
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			sHTML =	"<tr><td align='center'	onmouseover='this.style.backgroundColor=\"#f9f9f9\"' onmouseout='clearInterval(intervalID1);this.style.backgroundColor=\"\"' style='cursor:pointer; text-align:center; height:24px; line-height:24px;'	onmousedown='clearInterval(intervalID1);intervalID1=setInterval(\"decYear()\",30)' onmouseup='clearInterval(intervalID1)'>-</td></tr>";
			...
			for	(var i=(yearSelected-3); i<=(yearSelected+3); i++) {
				...
				sHTML += "<tr><td id='y" + j + "' onmouseover='this.style.backgroundColor=\"#f9f9f9\"' onmouseout='this.style.backgroundColor=\"\"' style='cursor:pointer; text-align:center; height:24px; line-height:24px;' onclick='selectYear("+j+");event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td></tr>";
				j ++;
			}

			sHTML += "<tr><td align='center' onmouseover='this.style.backgroundColor=\"#f9f9f9\"' onmouseout='clearInterval(intervalID2);this.style.backgroundColor=\"\"' style='cursor:pointer; text-align:center; height:24px; line-height:24px;;' onmousedown='clearInterval(intervalID2);intervalID2=setInterval(\"incYear()\",30)'	onmouseup='clearInterval(intervalID2)'>+</td></tr>";

			document.getElementById("selectYear").innerHTML	= "<table style='width:50px; font-size:11px; padding-left:0; border:1px solid #ddd;'	bgcolor='white' onmouseover='clearTimeout(timeoutID2)' onmouseout='clearTimeout(timeoutID2);timeoutID2=setTimeout(\"popDownYear()\",100)' cellspacing=0>"	+ sHTML	+ "</table>";
   ...
   function WeekNbr(n) {
	...
	function constructCalendar() {
		...
		sHTML =	"<table	 border=0 style='font-size:11px;'><tr>";

		if (showWeekNumber==1){
			sHTML += "<td width=25><b>" + weekString + "</b></td><td width=1 rowspan=7 bgcolor='#d0d0d0' style='padding:0px'><img src='"+imgDir+"divider.png' width=1></td>";
		}

		for	(var i=0; i<7; i++)	{
			if(dayName[i]=="일"){
				sHTML += "<td width='27' style='text-align:center;'><B><font color='#ea0530'>"+ dayName[i]+"</font></B></td>";
			}else if(dayName[i]=="토"){
				sHTML += "<td width='27' style='text-align:center;'><B><font color='#0054FF'>"+ dayName[i]+"</font></B></td>";
			}else{
				sHTML += "<td width='27' style='text-align:center;'><B>"+ dayName[i]+"</B></td>";
			}
		}
		sHTML +="</tr><tr>";
